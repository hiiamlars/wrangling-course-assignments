...
- Why are there different number of rows in the three data frames when this data all comes from the same participants? Maybe the data was cleaned manually before it was loaded into R
- Why are the numbers not round? I do not know what is meant by "round". They are complete numbers so that each could represent a natural number of participants with sufficient data
...
How would you bring up the help menu to understand how `janitor::clean_names()` works? ?janitor::clean_names()
...
What is the difference between select and filter? Selcted keeps the specified columns of a dataset meanwhile filter keeps the rows of a dataset that fullfil the specified condition

Which is for rows and which is for columns? Select is for columns, filter is for rows
...
What other ways are there of implementing this mutate, e.g., without using `%in%`? What are the pros and cons of each? One way is to use `case_when()` and specify the numerical formating should only be done in the specific cases and otherwise the value should be replaced with an NA. First, this approach is more explicit than the one above. This helps to understand the code. Therefore the code takes longer to write in the first place. Secondly, this code procduces a warning message which might confuse beginers they might have done something wrong.
...
This is easier to show than explain. What do you expect the output of the below chunk to be? And what is the actual output? After the explanation I expect R to round each line down to the next lower integer, respectively giving the first calcuation the result 1, the second one the result 2 and so on.
I was wrong.
R did not rounded down to the next lower integer all the time. Instead, R rounded every time to the closer even integer.
...
Mutate calculates a new value for each row or each group of the dataframe and hands out this new value for each row (of the respective group). Summarize will collapse all rows of the dataframe or all rows of the group into one row for the dataframe or for the group. Given the syntax above, mutate will calculate the mean of `age` for each row singely meanwhile summarize will take all all entries of `age` at once and calculate one mean age across the whole dataframe.
